...
const handleFile = () => {
    formFile.post(route('companies.filesVariables', props.company.id), {
        onFinish: () => {
            formFile.reset('file');
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="grid place-content-center">
            ...
            <div class="grid w-full max-w-sm items-center gap-1.5 mb-3">
                <Label for="picture">Variables</Label>
                <Input id="file" type="file" @input="formFile.file = $event.target.files[0]"/>
            </div>

            <Button :disabled="!formFile.file" @click="handleFile">Subir Variables</Button>
        </div>
    </AuthenticatedLayout>
</template>
