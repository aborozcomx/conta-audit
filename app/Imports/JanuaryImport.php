...
class JanuaryImport implements ToCollection, WithHeadingRow, WithChunkReading, ShouldQueue
{
    ...
    {
        $year = $this->year;
        $uma = Uma::where('year', $year - 1)->first();
        $batch = [];
        ...
        foreach ($rows as $row) {
            ...
            $employee = Employee::with([
                'company',
                'employee_salaries' => fn($q) => $q->where('year', $year)->where('period', 1)
        ...
        if (!empty($batch)) {
            DB::table('employee_quotas')->insert($batch);
        }
    }
}
