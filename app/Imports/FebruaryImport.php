...
class FebruaryImport implements ToCollection, WithHeadingRow, WithChunkReading, WithStartRow
{
    /**
        ...
        foreach ($rows as $row) {
            if ($row['sdi']) {
                ...
                if ($employee) {
                    ...
                    $salary = $employee->employee_salaries->where('year', $year)->where('period', 2)->first();
