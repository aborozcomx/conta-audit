...
class OctoberImport implements ToCollection, WithHeadingRow, WithChunkReading, WithStartRow
{
    /**
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            if ($row['sdi']) {
                ...
                if ($employee) {
                    $days = $row['dias'];
                    $salary = $employee->employee_salaries->where('year', $year)->where('period', 10)->first();
                    ...
                    if($salary) {
                        ...
                        $subtotal = $row['cuota_imss'] || 0; //$grouped[$row['nss']];
