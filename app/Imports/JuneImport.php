...
class JuneImport implements ToCollection, WithHeadingRow, WithChunkReading, WithStartRow
{
    /**
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            if ($row['sdi']) {
                ...
                if ($employee) {
                    $days = $row['dias'];
                    $salary = $employee->employee_salaries->where('year', $year)->where('period', 6)->first();
